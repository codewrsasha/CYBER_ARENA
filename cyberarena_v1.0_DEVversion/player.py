...
class Player(pygame.sprite.Sprite):
    ...
    def shoot(self, target_pos, bullet_group):
        """Стрельба с учетом бесконечных патронов"""
        from bullet import Bullet, LaserBeam
        
        current_time = pygame.time.get_ticks()
        
        # Проверяем перезарядку (если не активен бесконечный боезапас)
        if not self.infinite_ammo_active and self.is_reloading:
            return False
        ...
        # Проверяем патроны (если не активен бесконечный боезапас)
        ...
        # Уменьшаем патроны (только если не бесконечные)
        if not self.infinite_ammo_active:
            self.weapon_data["ammo"] -= 1
            
        self.last_shot_time = current_time
        
        # Автоматическая перезарядка если патроны кончились (и не бесконечные)
        if not self.infinite_ammo_active and self.weapon_data["ammo"] <= 0:
            self.start_reload()
