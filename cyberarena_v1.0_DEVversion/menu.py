...
class Menu:
    ...
    def main_menu(self):
        # Создаем переключатель языка в правом нижнем углу
        lang_switch = LanguageSwitch(self.settings.screen_width - 150, 
                                    self.settings.screen_height - 50, 130, 45)  # Увеличил размер
        ...
        while True:
            ...
            # Кнопки меню (локализованные)
            play_button = GlitchText("play", 
                                    self.settings.screen_width // 2, 300, 48,
                                    self.neon_red, True)
            settings_button = GlitchText("settings",
                                        self.settings.screen_width // 2, 400, 48,
                                        self.neon_red, True)
            exit_button = GlitchText("exit",
                                    self.settings.screen_width // 2, 500, 48,
                                    self.neon_red, True)
