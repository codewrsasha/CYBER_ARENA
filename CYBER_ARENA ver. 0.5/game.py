...
class Game:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    self.running = False
                elif event.key == pygame.K_m:  # Клавиша M для выключения музыки
            ...
            # Обработка стрельбы
                ...
                if event.button == 1:  # Левая кнопка мыши
                    ...
                    # Звук выстрела будет воспроизводиться в update при стрельбе
        ...
        for enemy in self.enemies:
            if self.player.rect.colliderect(enemy.rect):
                if self.player.take_damage(enemy.damage):
                    self.sound_manager.play_player_hit()  # Звук попадания по игроку
                    self.game_over()
                else:
                    self.sound_manager.play_player_hit()  # Звук попадания по игроку
            ...
            if isinstance(bullet, LaserBeam):
                ...
                for enemy in hit_enemies:
                    if enemy.take_damage(self.player.bullet_damage):
                        self.sound_manager.play_hit()  # Звук попадания
                        self.score += 10
                        enemy.kill()
            else:
                # Для обычных пуль
    ...
    def run(self):
        while self.running:
            ...
            # Рисуем игрока (используем текущий спрайт из player.image)
