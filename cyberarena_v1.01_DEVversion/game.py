...
class Game:
    def __init__(self, screen, settings=None):
        ...
        # Инициализация звуков (передаем настройки)
        ...
        # Добавляем меню паузы
        self.pause_menu = PauseMenu(screen, self.settings)
        ...
        # Создание игрока в центре арены (game_speed=True для основной игры)
        self.player = Player(self.settings.ARENA_WIDTH // 2,
                            self.settings.ARENA_HEIGHT // 2,
                            game_speed=True)  # Добавлен параметр
        ...
        self.session_kills_by_type = {"trojan": 0, "worm": 0, "ransomware": 0}
        self.session_damage_by_weapon = {"standard": 0, "rapid": 0, "shotgun": 0, "laser": 0}
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            # Обработка стрельбы (только если игра не на паузе)
    ...
    def restart_game(self):
        """Перезапускает игру с сохранением настроек"""
        # Сохраняем настройки звука
        music_vol = self.sound_manager.music_volume
        sfx_vol = self.sound_manager.sfx_volume
        ...
        self.sound_manager.set_sfx_volume(sfx_vol)
                    
    def update(self):
        # Обновляем игру только если не на паузе
        if not self.paused:
            # ... весь существующий код update() ...
            self.player.update()
            self.enemies.update()
            self.bullets.update()
            ...
            self.enemy_spawner.set_wave(self.wave)
    ...
    def check_collisions(self):
        ...
        for bullet in self.bullets:
            # Получаем текущий урон (с проверкой)
            current_damage = self.player.weapon_data["damage"] if hasattr(self.player, 'weapon_data') else 15
            ...
            if isinstance(bullet, LaserBeam):
                ...
                for enemy in hit_enemies:
                    if enemy.take_damage(current_damage):
                        ...
                        self.session_kills_by_type[enemy.virus_type] = \
                            self.session_kills_by_type.get(enemy.virus_type, 0) + 1
                        self.session_damage_by_weapon[self.player.current_weapon] = \
                            self.session_damage_by_weapon.get(self.player.current_weapon, 0) + current_damage
    ...
    def run_game(self):  # Переименовали с run на run_game
        ...
        # После завершения игры сохраняем статистику
        statistics.update_session_stats(
            self.wave,
            self.score,
            self.session_kills,
            self.session_kills_by_type,
            self.session_damage_by_weapon
        )